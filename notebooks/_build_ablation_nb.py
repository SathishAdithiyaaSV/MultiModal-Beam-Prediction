...
RUNS = OUT / "runs"
# Optional: the other part's results, attached as a Kaggle dataset, so this
# notebook can produce the complete merged table.
MERGE_PARTNER_RESULTS = True
...
Reads every per-configuration result present, including part {other}'s if you
attached it as a Kaggle dataset. With only one part attached the table is
partial and says so.
"""))
    C.append(code(rf"""
def load_all_records():
    '''Every per-configuration result available, this part's and the partner's.'''
    found, seen = [], set()
    for path in sorted((OUT / "per_config").glob("*.json")):
        r = json.loads(path.read_text())
        found.append(r)
        seen.add(r["slug"])
    if MERGE_PARTNER_RESULTS:
        for d, _dirs, files in walk_dirs(INPUT_ROOT):
            if d.name != "per_config":
                continue
            for f in sorted(files):
                if not f.endswith(".json"):
                    continue
                r = json.loads((d / f).read_text())
                if r.get("slug") and r["slug"] not in seen:
                    found.append(r)
                    seen.add(r["slug"])
                    print(f"merged partner result: {{r['label']}}")
    return found
...
def flatten(r, split="val"):
    ...
    return {{
        ...
        "Top-5": m.get("top5"), "DBA": m.get("dba"),
        "n": m.get("n"),
...
records_all = load_all_records()
results = pd.DataFrame([flatten(r) for r in records_all])
results["_o"] = results.slug.apply(lambda s: ORDER.index(s) if s in ORDER else 99)
results = results.sort_values("_o").drop(columns="_o").reset_index(drop=True)

complete = len(results) == 8
print(f"{{len(results)}}/8 configurations available"
      f"{{'' if complete else '  -- PARTIAL, run the other part for the full table'}}\n")
results.to_csv(OUT / f"results_part{{PART}}.csv", index=False)
if complete:
...
"""))
    ...
    C.append(md(r"""
## 10. Incremental value over the cheap baseline

The number the adaptive-routing design actually needs: **what does each
expensive modality buy on top of GPS + Radar?**
"""))
    C.append(code(r"""
BASE = "gps_radar"
deltas = pd.DataFrame()
if BASE in set(results.slug):
    base = results[results.slug == BASE].iloc[0]
    rows = []
    for _, r in results.iterrows():
        if r.slug == BASE:
            continue
        added = [m for m in r.slug.split("_") if m not in base.slug.split("_")]
        if not added:
            continue
        rows.append({
            "Added to GPS + Radar": " + ".join(
                {"image": "Camera", "lidar": "LiDAR", "radar": "Radar"}.get(a, a.upper())
                for a in added),
            "Configuration": r["Configuration"],
            "ΔTop-1": r["Top-1"] - base["Top-1"],
            "ΔTop-3": r["Top-3"] - base["Top-3"],
            "ΔDBA": r["DBA"] - base["DBA"],
            "ΔParams (M)": r["Params (M)"] - base["Params (M)"],
            "ΔLatency (ms)": r["Latency (ms)"] - base["Latency (ms)"],
        })
    deltas = pd.DataFrame(rows).sort_values("ΔDBA", ascending=False)
    deltas.to_csv(OUT / f"deltas_part{PART}.csv", index=False)
    print(f"baseline: {base['Configuration']}  "
          f"Top-1 {base['Top-1']:.4f}  Top-3 {base['Top-3']:.4f}  DBA {base['DBA']:.4f}\n")
    print(deltas.round(4).to_string(index=False))
else:
    print("GPS + Radar not available yet -- it is trained in part 1. "
          "Deltas will appear once that result is present.")
"""))

    C.append(md(r"""
## 11. Scenario-wise results
...
if not scenario_results.empty:
    ...
    print("no key configurations available yet")
"""))

    C.append(md("## 12. Plots"))
    C.append(code(r"""
...
plt.tight_layout()
plt.savefig(PLOTS / f"ablation_part{PART}.png", dpi=150)
plt.show()

if not deltas.empty:
    fig, ax = plt.subplots(1, 2, figsize=(15, 4.2))
    ax[0].barh(deltas["Added to GPS + Radar"], deltas["ΔDBA"], color="tab:red")
    ax[0].axvline(0, color="k", lw=.8)
    ax[0].set(xlabel="ΔDBA vs GPS + Radar", title="Incremental value over the cheap baseline")
    ax[0].grid(alpha=.3, axis="x")
    ax[0].invert_yaxis()

    ax[1].scatter(results["Latency (ms)"], results["DBA"], s=70, color="tab:purple")
    for _, r in results.iterrows():
        ax[1].annotate(r.Configuration.replace(" + ", "+"),
                       (r["Latency (ms)"], r["DBA"]), fontsize=8,
                       xytext=(4, 3), textcoords="offset points")
    ax[1].set(xlabel="inference latency (ms/sample)", ylabel="DBA",
              title="Performance vs inference cost")
    ax[1].grid(alpha=.3)
    plt.tight_layout()
    plt.savefig(PLOTS / f"deltas_and_cost_part{PART}.png", dpi=150)
    plt.show()
print(f"plots -> {PLOTS}")
"""))
...
Generated from the measured results only. It states what the numbers show,
including when they show that adaptive routing is *not* worth pursuing.
"""))
    C.append(code(r"""
def interpret(results, deltas, scenario_results):
    L, have = [], set(results.slug)
    if "gps_radar" not in have:
        return ["GPS + Radar is not available yet, so the comparisons this "
                "experiment exists to make cannot be drawn. Run part 1."]
    base = results[results.slug == "gps_radar"].iloc[0]
    gps = results[results.slug == "gps"].iloc[0] if "gps" in have else None
    full = (results[results.slug == "gps_radar_image_lidar"].iloc[0]
            if "gps_radar_image_lidar" in have else None)

    # 1. is the cheap baseline strong?
    if gps is not None:
        d = base["DBA"] - gps["DBA"]
        L.append(f"**Radar over GPS alone.** GPS-only reaches DBA {gps['DBA']:.4f} "
                 f"(Top-1 {gps['Top-1']:.4f}); adding radar moves it to "
                 f"DBA {base['DBA']:.4f} (Top-1 {base['Top-1']:.4f}), a gain of "
                 f"{d:+.4f} DBA for {base['Latency (ms)'] - gps['Latency (ms)']:+.2f} ms.")
    if full is not None:
        gap = full["DBA"] - base["DBA"]
        frac = base["DBA"] / full["DBA"] if full["DBA"] else float("nan")
        verb = "retains" if frac <= 1.0 else "already exceeds"
        L.append(f"**How cheap is cheap.** GPS + Radar {verb} "
                 f"{frac * 100:.1f}% of the full model's DBA at "
                 f"{base['Latency (ms)'] / full['Latency (ms)'] * 100:.0f}% of its latency "
                 f"and {base['Params (M)'] / full['Params (M)'] * 100:.0f}% of its active "
                 f"parameters. The full model is {gap:+.4f} DBA "
                 + ("better." if gap > 0 else
                    "WORSE -- the extra modalities are not paying for themselves "
                    "at this training budget, which must be reported as-is."))

    # 2/3. what does each expensive modality add?
    def added(slug):
        return (results[results.slug == slug].iloc[0] if slug in have else None)

    cam, lid = added("gps_radar_image"), added("gps_radar_lidar")
    if cam is not None and lid is not None:
        dc, dl = cam["DBA"] - base["DBA"], lid["DBA"] - base["DBA"]
        L.append(f"**Camera vs LiDAR on top of GPS + Radar.** Camera "
                 f"{dc:+.4f} DBA ({cam['Latency (ms)'] - base['Latency (ms)']:+.2f} ms); "
                 f"LiDAR {dl:+.4f} DBA "
                 f"({lid['Latency (ms)'] - base['Latency (ms)']:+.2f} ms).")
        big, small = (("Camera", dc), ("LiDAR", dl)) if dc >= dl else (("LiDAR", dl), ("Camera", dc))
        if max(dc, dl) < 0.01:
            L.append(f"  Both gains are under 0.01 DBA. On this evidence neither "
                     f"expensive modality is worth selectively acquiring, and an "
                     f"adaptive router over them would have little to gain. "
                     f"**This argues against pursuing adaptive routing as framed.**")
        elif abs(dc - dl) < 0.005:
            L.append(f"  The two are within 0.005 DBA of each other, so neither "
                     f"dominates. Both stay as candidate escalation routes, and the "
                     f"choice should be made on cost: "
                     f"Camera {cam['Latency (ms)']:.2f} ms vs LiDAR {lid['Latency (ms)']:.2f} ms.")
        else:
            L.append(f"  {big[0]} is the clearly more valuable escalation "
                     f"({big[1]:+.4f} vs {small[1]:+.4f} DBA), so the adaptive "
                     f"experiment should prioritise it as the first modality to acquire.")

    # 4. does the full model justify itself?
    if full is not None and cam is not None:
        best_single = max([x for x in (cam, lid) if x is not None],
                          key=lambda r: r["DBA"])
        extra = full["DBA"] - best_single["DBA"]
        L.append(f"**Is the fourth modality worth it?** Going from "
                 f"{best_single['Configuration']} to the full model adds "
                 f"{extra:+.4f} DBA for "
                 f"{full['Latency (ms)'] - best_single['Latency (ms)']:+.2f} ms. "
                 + ("That is marginal, which supports escalating to one expensive "
                    "modality rather than all of them."
                    if extra < 0.01 else
                    "That is a real gain, so full escalation should remain an option "
                    "in the routing policy."))

    # 5. consistency across environments
    if not scenario_results.empty:
        piv = scenario_results[scenario_results.split == "val"].pivot_table(
            index="scenario", columns="slug", values="DBA")
        if "gps_radar" in piv.columns:
            per = {}
            for slug, name in (("gps_radar_image", "Camera"), ("gps_radar_lidar", "LiDAR")):
                if slug in piv.columns:
                    per[name] = (piv[slug] - piv["gps_radar"]).round(4).to_dict()
            if per:
                bits = "; ".join(
                ...
                L.append(f"**Consistency across environments (val, ΔDBA vs GPS+Radar).** "
                         f"{bits}. Spread across scenarios is {spread:.3f} DBA"
                         + (", so the benefit is roughly uniform and a global policy "
                            "may suffice." if spread < 0.02 else
                            ", so the benefit is environment-dependent -- which is "
                            "itself an argument for routing per sample rather than "
                            "a fixed modality set."))

    L.append("**Caveats.** Trained for "
             f"{int(results['Best epoch'].max()) if len(results) else 0}-epoch-scale "
             "budgets rather than to full convergence; historical beam indices "
             "disabled; scenario 31 rests on 50 adaptation samples; and scenario 34 "
             "supplies radar for only ~40% of its samples, so radar configurations "
             "are partly GPS-only there.")
    return L


lines = interpret(results, deltas, scenario_results)
print("\n".join(f"{i}. {t}" if not t.startswith("  ") else t
                for i, t in enumerate(lines, 1)))
(OUT / f"interpretation_part{PART}.md").write_text("\n\n".join(lines))
"""))

    C.append(md("## 14. Save outputs"))
    C.append(code(rf"""
config_record = {{
    "part": PART,
    ...
    "n_configurations_available": int(len(results)),
}}
(OUT / f"config_part{{PART}}.json").write_text(json.dumps(config_record, indent=2))
(OUT / f"summary_part{{PART}}.json").write_text(json.dumps(
    {{"config": config_record, "results": results.to_dict(orient="records")}}, indent=2))
...
print(f"\nTo merge with part {other}: download this part's outputs/modality_ablation/"
      f"per_config/*.json, upload them as a Kaggle dataset, attach it to the other "
      f"notebook, and re-run its aggregation cells. Either notebook then produces "
      f"the complete 8-configuration table.")
"""))
