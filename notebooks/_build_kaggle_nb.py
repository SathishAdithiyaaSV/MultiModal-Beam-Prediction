...
# The loader derives its root as index_dir.parents[2]. Verify against every
# split and a sample of rows, not just one: a dataset re-upload can easily
# refresh one source and leave another stale or incomplete, and that must fail
# here rather than hours later in the predict cells.
_s = pd.read_csv(INDEX_DIR / "samples.csv")
ROOT = INDEX_DIR.parents[2]
# (column prefix, its availability-mask column)
MODALITY_GROUPS = [("image", "m_image"), ("lidar_bev", "m_lidar"), ("radar", "m_radar")]

print(f"derived root: {ROOT}\n")
print(f"{'split':<18}{'rows':>7}{'checked':>9}{'missing':>9}")
problems = {}
for split in ("train", "val", "adaptation", "test"):
    rows = _s[_s.split == split]
    if rows.empty:
        print(f"{split:<18}{0:>7}{'-':>9}{'-':>9}   (absent)")
        continue
    probe = rows.sample(min(25, len(rows)), random_state=0)
    # Only require a file to exist where the eq. (3) mask says the modality is
    # available. Scenario 34 legitimately has no radar for ~60% of its samples,
    # and the loader returns zeros for those rather than reading the disk.
    missing, n_checked = [], 0
    for _, r in probe.iterrows():
        for prefix, mask_col in MODALITY_GROUPS:
            if not r[mask_col]:
                continue
            for i in range(1, 6):
                n_checked += 1
                col = f"{prefix}_{i}"
                if not (ROOT / r[col]).exists():
                    missing.append((r.sample_id, col))
    print(f"{split:<18}{len(rows):>7}{n_checked:>9}{len(missing):>9}")
    if missing:
        problems[split] = missing

if problems:
    print("\nMISSING FILES — the attached dataset does not match the index:")
    for split, items in problems.items():
        for sid, col in items[:5]:
            print(f"  {split}: {sid}  {col} -> {ROOT / _s.set_index('sample_id').loc[sid, col]}")
        if len(items) > 5:
            print(f"  ... and {len(items) - 5} more in {split}")
    raise SystemExit(
        "Staging failed. Most likely a dataset version refreshed some sources but "
        "not others, so samples.csv references files that were not uploaded. "
        "Re-upload the source(s) listed above together with index/.")

# per-scenario counts on disk, to catch a source that uploaded only partially
print("\nfiles found per source/scenario:")
for name, src in sorted(SOURCE_DIRS.items()):
    if src is None:
        continue
    for scn in sorted(src.glob("scenario*")):
        counts = {m: len(list((scn / m).glob("*"))) if (scn / m).is_dir() else 0
                  for m in ("camera", "lidar_bev", "radar_ra_rv")}
        print(f"  {name:<12}{scn.name:<12}" +
